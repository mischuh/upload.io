...
def auto_str(__repr__=False):
    ...
    Args:
        __repr__ (bool): If set to true, the decorator will 
        auto-implement the __repr__() method as well.
    Returns:
        callable: Decorating function.
    Note:
        There are known issues with self referencing (self.s = self). 
        Recursion will be identified by the python interpreter and will 
        do no harm, but it will actually not work.
    ...
    def decorator(cls):
        def __str__(self):
            items = ["{name}={value}".format(
                ...
                if name not in get_field_mro(
                    self.__class__, '__auto_str_ignore__')]
...
class Loggable:
    """
    Adds a logger property to the class to provide easy 
    access to a configured logging instance to use.
    Example:
